...
function App() {
  ...
  const [currentlyPlaying, setCurrentlyPlaying] = useState<{[channelNumber: number]: VisualizedAudioItem | null}>({});
  ...
  useEffect(() => {
    const updateVisualizer = (channelNumber: number) => {
    ...
    <div className="App">
      <div className='example-section'>
        ...
        <div className='example-block-columns'>
          <MultiChannelExampleBlock 
            example={addSoundToQueueExample} 
            isChannelZeroQueueEmpty={queueState[0]}
            isChannelOneQueueEmpty={queueState[1]}
          />
          <MultiChannelExampleBlock 
            example={stopCurrentSoundAndPlayNextExample} 
            isChannelZeroQueueEmpty={queueState[0]}
            isChannelOneQueueEmpty={queueState[1]}
          />
          <MultiChannelExampleBlock 
            example={stopSoundAndEmptyQueueExample} 
            isChannelZeroQueueEmpty={queueState[0]}
            isChannelOneQueueEmpty={queueState[1]}
          />
          <MultiChannelExampleBlock 
            example={stopAllSoundsInAllChannelsExample} 
            isChannelZeroQueueEmpty={queueState[0]}
            isChannelOneQueueEmpty={queueState[1]}
          />
        </div>
